# CMPT 120 Final Project - Machine Learning Part 2 - Building a Model Using Real Data
# Author = Nathan Hare
# Date = Nov.22nd, 2020 (Started)
# Date = Nov. 26th, 2020 (Completed)
# Data File in question = SeoulBikeData.csv
...
# ---- OUTPUT DATA ---- #
with open('SeoulBikeData.csv', 'r') as bikeDataFileOutput:
    bikeDataFileOutput.readline() # remove the header line from the file

    outputBikeData = [] # create an empty list to append the output data to
    linesOutputBikeData = bikeDataFileOutput.readlines()

    # Split lines into sublists per each line based on a comma separator
    for lines in linesOutputBikeData:
        linesOutputSplit = lines.split(',')
        outputBikeData.append(linesOutputSplit[1])

    # create 2 lists to split our output data into
    trainOutputData = outputBikeData[:int(len(outputBikeData)*0.8)]
    testOutputData = outputBikeData[-int(len(outputBikeData)*0.2):]
...
# ---- INPUT DATA ---- #
with open('SeoulBikeData.csv', 'r') as bikeDataFileInput:
    bikeDataFileInput.readline() # remove the header line from the file
    inputBikeData = []
    linesInputBikeData = bikeDataFileInput.readlines()

    # Split lines into sublists per each line based on a comma separator
    for lines in linesInputBikeData:
        linesInputSplit = lines.split(',')
        inputBikeData.append(linesInputSplit[2:11])

    # create 2 lists to split our output data into
    trainInputData = inputBikeData[:int(len(inputBikeData)*0.8)]
    testInputData = inputBikeData[-int(len(inputBikeData)*0.2):]

    n = 0
    trainInputFloat = [] # define empty list to append to later
    ...
    n = 0
    testInputFloat = [] # define empty list to append to later
...
# define a dictionary using the previously set variables
errorDict = {'less10': 0,
            '10to20': 0,
            '20to30': 0,
            '30to40': 0,
