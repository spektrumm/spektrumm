# CMPT 120 Final Project - Machine Learning Part 4 - Second Project Extension
# Author = Nathan Hare
# Date = Nov.22nd, 2020 (Started)
# Date = Nov. 29th, 2020 (Completed)
# Data file provided = User provided data file
...
# all of the inputs are assuming that the user provides valid input #

# ask the user input regarding the file name
print('Include the ".csv" extension in your input')
fileNameInput = input('Please enter the desired filename: ')

# ask the user for input regarding line indexing
outputIndex = int(input('Enter the Column Number of the output secton: '))
adjOutIndex = outputIndex - 1
inputRangeStart = int(input('Enter the start of the input index range: '))
inputRangeEnd = int(input('Enter the end of the input index range: '))
adjInRS = inputRangeStart - 1
adjInRE = inputRangeEnd - 1

separatorCheck = input('What type of separator is used in the file?: ')

# ask the user if the file contains a header line
headerLineCheck = input('Does the file contain a header line? (Y/N): ').lower()

# ---- OUTPUT DATA ---- #
with open(fileNameInput, 'r') as DataFileOutput:

    if headerLineCheck == 'yes' or headerLineCheck == 'y':

        DataFileOutput.readline() # remove the header line from the file
        print('log: header present in File - Removed (Output)')
    else:
        print('log: No header present in File')

    outputCSVData = []
    linesOutputCSVData = DataFileOutput.readlines()

    # Split lines into sublists per each line based on a comma separator
    for lines in linesOutputCSVData:
        linesOutputSplit = lines.split(separatorCheck)
        outputCSVData.append(linesOutputSplit[adjOutIndex])

    # create 2 lists to split our output data into
    trainOutputData = outputCSVData[:int(len(outputCSVData)*0.8)]
    testOutputData = outputCSVData[-int(len(outputCSVData)*0.2):]
...
# ---- INPUT DATA ---- #
with open(fileNameInput, 'r') as DataFileInput:

    if headerLineCheck == 'yes' or headerLineCheck == 'y':

        DataFileInput.readline() # remove the header line from the file
        print('log: header present in File - Removed (Input)')
    else:
        print('log: No header present in File')

    inputCSVData = []
    linesInputCSVData = DataFileInput.readlines()

    # Split lines into sublists per each line based on a comma separator
    for lines in linesInputCSVData:
        linesInputSplit = lines.split(',')
        inputCSVData.append(linesInputSplit[adjInRS:adjInRE])

    # create 2 lists to split our output data into
    trainInputData = inputCSVData[:int(len(inputCSVData)*0.8)]
    testInputData = inputCSVData[-int(len(inputCSVData)*0.2):]
...
n = 0
percentErrorList = [] # define an empty list to append to later
for value in range(len(noZeroTest)):
    
    calcError = ((abs(noZeroTest[n] - noZeroResultList[n])) / noZeroTest[n])*100
    percentErrorList.append(calcError)

    n += 1

# define a set of variables ranging from 
# <10% to >10% for % error cateogorizing
LessThan10 = 0
_10to20 = 0
_20to30 = 0
_30to40 = 0
...
# define conditional statements to assign a dynamic
# y axis label multiplier based off the amount of items predicted
yLabelMult = 1
if len(percentErrorList) <= 250:
    yLabelMult += 0
elif len(percentErrorList) > 250 and len(percentErrorList) <= 500:
    yLabelMult += 1
elif len(percentErrorList) > 500 and len(percentErrorList) <= 1500:
    yLabelMult += 9
else:
    # default to an axis scale of 100 if len(percentErrorList)
    # does not fit a predefined category.
    print('log: length of list does not fit predefined category.')
    yLabelMult += 9

print(f'log: chosen yLabelMult: {yLabelMult}')

# define conditional statements to assign a dynamic
# barLen multiplier based off the amount of items predicted
yValueMult = 1
if len(percentErrorList) <= 250:
    yValueMult += 9
elif len(percentErrorList) > 250 and len(percentErrorList) <= 500:
    ...
    # default to a large bar scale if len(percentErrorList)
    # does not fit a predefined category.
    print('log: length of list does not fit predefined category.')
    yValueMult += 0

print(f'log: chosen yValueMult = {yValueMult}')
...
for category, percAmt in errorDict.items():

    drawBar(percAmt * yValueMult)
    nextBar()
...
for yLabel in range(len(graphYLabels)):
    ...
    tGraph.forward(100)
    tGraph.write((graphYLabels[n]) * yLabelMult)
